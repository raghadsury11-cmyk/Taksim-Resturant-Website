...
function updateThemeButton(isDark) {
    ...
    if (isDark) {
        themeToggleBtn.innerHTML = `<span class="theme-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg></span><span class="theme-label">نهاري</span>`;
    } else {
        themeToggleBtn.innerHTML = `<span class="theme-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg></span><span class="theme-label">ليلي</span>`;
    }
}

function applySavedTheme() {
    const isDark = localStorage.getItem('siteTheme') === 'dark';
    if (isDark) { document.body.setAttribute('data-theme', 'dark'); } else { document.body.removeAttribute('data-theme'); }
...
function toggleMobileNav() {
    ...
    // Scroll to top smoothly when opening the mobile navigation
...
// ================= ACTIVE FILTER STATE =================
// Single source of truth — no relying on URL for filter logic
let activeFilter = 'all-category';

// ================= APPLY FILTERS (uses activeFilter directly) =================
function applyFilters() {
    const searchInput = document.getElementById('menu-search-input')?.value.toLowerCase().trim() || '';
    ...
    allCards.forEach(card => {
        ...
        const filterMatch =
            ...
            cardSection.split(' ').includes(activeFilter) ||
            cardTag.split(' ').includes(activeFilter);
...
// ================= SET FILTER (updates state + UI + applies) =================
function setFilter(filterValue) {
    activeFilter = filterValue;

    // Update URL for bookmarking (doesn't affect filter logic)
...
function showCategoryWrapper(filterValue) {
    ...
    const searchAndTabs = document.getElementById('search-and-tabs');
    if (searchAndTabs) {
        searchAndTabs.style.display = filterValue === 'all-category' ? 'block' : 'none';
    }
...
function filterCategoryTags(tag, event) {
    const filterValue = tag === 'all-tags' ? 'all-category' : tag;
    document.querySelectorAll('.cat-tab-btn').forEach(btn => btn.classList.remove('active'));
...
function toggleCardFlip(cardInnerElement) {
    ...
    document.querySelectorAll('.dish-card-inner.flipped').forEach(el => el.classList.remove('flipped'));
    if (!isFlipped) { cardInnerElement.classList.add('flipped'); }
}
...
function submitContactForm(event) {
    ...
    alert('تم إرسال رسالتك بنجاح! شكرًا لتواصلك مع مطعم تقسيم.');
...
function renderCartPopupList() {
    ...
    shoppingCart.forEach((item, index) => {
        ...
        removeButton.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;
...
function initializeDishRemoveButtons() {
    document.querySelectorAll('.dish-card .add-to-cart-btn').forEach(addButton => {
        ...
        removeButton.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/></svg>إزالة من السلة`;
...
window.addEventListener('scroll', () => {
    ...
    scrollTopBtn.style.display = (document.documentElement.scrollTop > 400) ? 'flex' : 'none';
});
...
function initTestimonialsCarousel() {
    ...
    track.addEventListener('touchend', () => setTimeout(() => isPaused = false, 1500), { passive: true });
    function animate() {
        if (!isPaused) {
            offset += speed;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Only run filter logic on menu page
    if (document.getElementById('full-menu-view')) {
        ...
        const searchAndTabs = document.getElementById('search-and-tabs');
        if (searchAndTabs) {
            searchAndTabs.style.display = initialFilter === 'all-category' ? 'block' : 'none';
        }

        // Set activeFilter then apply — no race condition
